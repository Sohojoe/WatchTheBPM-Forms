...
namespace BPM
{
	public class BPMModel : INotifyPropertyChanged 
	{
		...
		public uint AveBPM { get { return Math.Min((uint)Math.Round(MeanBPM), 999);} }
		public string AveBPMStr { get { return AveBPM.ToString();} }
		...
		public void RecordTap ()
		{
			...
			if (lenght < kMaxTime) {
				...
				Debug.WriteLine (string.Format("Mean:{0} Median:{1} Varance:{2} StdDev:{3} Last:{4}",Math.Round (MeanBPM),Math.Round (MedianBPM),Math.Round (VarianceBPM, 2), Math.Round (StdDevBPM,2), Math.Round(bpm)) );
				OnPropertyChanged (nameof (AveBPM));
				OnPropertyChanged (nameof (MeanBPM));
				OnPropertyChanged (nameof (MedianBPM));
				OnPropertyChanged (nameof (StdDevBPM));
				OnPropertyChanged (nameof (AveBPMStr));
			}
			stopwatch.Restart();
		}
	}
